...
export const TemplateLibrary: React.FC<TemplateLibraryProps> = ({ onTemplateSelect }) => {
    ...
    useEffect(() => {
        // Load templates from localStorage + defaults
    ...
    const handleDelete = (id: string) => {
        if (confirm('¿Eliminar esta plantilla? Esta acción no se puede deshacer.')) {
            const updated = templates.filter(t => t.id !== id && !t.isDefault);
            setTemplates(updated.filter(t => !DEFAULT_SERVICE_TEMPLATES.find(dt => dt.id === t.id)));
            localStorage.setItem('customServiceTemplates', JSON.stringify(updated.filter(t => !t.isDefault)));
        }
    };
                ...
                <div className="flex flex-wrap gap-1">
                    {allTags.map(tag => (
                        <button
                            ...
                            className={`text-xs px-2 py-1 rounded-full transition-colors ${selectedTags.includes(tag)
